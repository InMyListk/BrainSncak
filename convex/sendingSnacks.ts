...
export const sendSnackEmailForUser = action({
  handler: async (ctx,args: {
    email: string;
      ...
      const userProfiles = await ctx.runQuery(api.userProfiles.getUserProfile, args);

      if (!userProfiles) {
        throw new Error("User profile not found");
      }

      const { email, preferences, snackStyle, language, subscription } = userProfiles;

      // Skip if missing required data
      if (!email || !preferences?.length || !snackStyle || !subscription) return;
