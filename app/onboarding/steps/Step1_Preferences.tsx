...
export default function Step1_Preferences({
  user,
  setUser,
  customTags,
  setCustomTags,
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <MultiSelect
          ...
          onChange={(selected) => {
            const newPredefined = selected.filter((s) =>
              availableTopics.some((t) => t.id === s)
            );
            const newCustomIds = selected.filter(
              (s) => !availableTopics.some((t) => t.id === s)
            );
      ...
      {allSelectedPreferences.length > 0 && (
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg">
          <p className="text-sm font-medium text-purple-900 mb-2">
            Selected {allSelectedPreferences.length} topic
            {allSelectedPreferences.length !== 1 ? "s" : ""}:
          </p>
          <p className="text-sm text-purple-700">
            You&rsquo;ll receive content about{" "}
